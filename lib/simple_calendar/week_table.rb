module SimpleCalendar
  class WeekTable < WeekCalendar
    ...
    def render_week_hour(week, hour)
      ...
      safe_join([content_tag(:th, I18n.l(Time.at(hour), format: :time_only), style: 'font-weight: normal')] + results)
    end
            ...
            capture do
              ([''] + dates).each do |date|
                if date.is_a?(Date)
                  ...
                  concat content_tag(:th, I18n.t('labels.meeting_time'), style: 'width: 50px')
                end
              end
            end
          end
